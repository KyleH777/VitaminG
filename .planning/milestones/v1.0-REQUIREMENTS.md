# Requirements Archive: Vitamin G v1.0

**Archived:** 2026-05-15
**Status:** ✅ MILESTONE COMPLETE — all 98 v1 requirements satisfied
**Defined:** 2026-04-03
...
- [x] **NOTIF-01**: App requests notification permission during onboarding (not on cold launch without context)
...
- [x] **SYNC-01**: All `Goal` and `CompletionEvent` data syncs across user's devices via CloudKit private database
- [x] **SYNC-02**: Sync works transparently — no manual sync button in v1
- [x] **SYNC-03**: CloudKit schema promoted to Production before App Store submission (deployment task)

### Widgets

- [x] **WIDGET-01**: Home screen widget (`systemMedium`) shows top active goals across tiers
- [x] **WIDGET-02**: Lock screen widget (`accessoryRectangular`) shows current global streak or top active goal title
- [x] **WIDGET-03**: Widget reads from shared App Group SwiftData store — same data as main app
- [x] **WIDGET-04**: Widgets are read-only in v1 — no write operations from widget context
- [x] **WIDGET-05**: Widget timeline refreshes at least once daily (aligns with morning notification)

### Onboarding

- [x] **ONBOARD-01**: First-launch onboarding explains the 4 tiers with warm, gratitude-framing copy
- [x] **ONBOARD-02**: Onboarding guides user to create their first goal before reaching the main view
- [x] **ONBOARD-03**: Notification permission request occurs during onboarding with clear value framing
- [x] **ONBOARD-04**: Empty states for each tier include actionable prompts to add a first goal
...
- [x] **UI-04**: App Store-quality polish: no placeholder UI, no debug elements, smooth transitions
...
- [x] **PROF-06**: App handles incoming `vitaming://profile/<recordID>` deep links via `.onOpenURL` handler in `VitaminGApp`
- [x] **PROF-07**: Programmatic navigation to a specific profile via deep link resolves and navigates correctly
...
- [x] **CHAL-13**: Community feed scoped per challenge category — posts (text + optional photo) visible only within the same category
- [x] **CHAL-14**: Post reactions: 👍 and ❤️ only — no comments, no other reaction types; reaction counts visible on post
- [x] **CHAL-15**: Report button present on every post; report count never shown publicly
- [x] **CHAL-16**: Profanity filter runs on post submission — rejects and prompts user to edit; never silently drops content
- [x] **CHAL-17**: Community posts and reactions persist in CloudKit public database
- [x] **CHAL-18**: Spending Freeze module — daily self-reported toggle, freeze badge on dashboard, daily reminder while active
- [x] **CHAL-19**: Craving Tools module — box breathing exercise (4-4-4-4 pattern), random motivational distraction prompt, buddy ping button
- [x] **CHAL-20**: Transformation Photos module — private dated photo log visible only to the user
- [x] **CHAL-21**: Nutrition Log module — simple daily meal note field per challenge
- [x] **CHAL-22**: Buddy Accountability module — user opts in a contact; buddy receives push ping on request
- [x] **CHAL-23**: Custom Challenge builder lets users configure name, category, check-in type, goal type/value, duration, and privacy — produces a ChallengeTemplate using identical infrastructure as featured challenges
- [x] **CHAL-24**: Notification suite: streak-at-risk (no check-in by 8pm), milestone reached, reaction received on post, buddy accountability ping
- [x] **CHAL-25**: Warm, encouraging empty state on community feed when sparse ("Be the first to share your progress"); no red failure states throughout challenge UI

### Gratitude / Daily Wins

- [x] **GRAT-01**: User can create a daily win or gratitude entry — free-text, date-keyed, one entry per calendar day
- [x] **GRAT-02**: `DailyWin` SwiftData model persists entries with `id` (UUID), `date` (Date?), `text` (String?) — CloudKit-compatible (all optional)
- [x] **GRAT-03**: A dedicated Daily Wins surface shows today's entry editor and a scrollable history of past entries with their dates
- [x] **GRAT-04**: If today's entry already exists, opening the Daily Wins view pre-fills the editor — no duplicate entries per day
- [x] **GRAT-05**: A "What's your win today?" notification variant fires at a user-configurable time, distinct from the goal-reminder notification
- [x] **GRAT-06**: Daily Wins is accessible from a prominent surface (dedicated tab or home screen shortcut) within the app

### Goal Progress Visualization

- [x] **PROG-01**: Each goal card displays a progress ring or activity bar derived from `CompletionEvent` records — fills proportionally to recent completion frequency
- [x] **PROG-02**: GoalDetailView shows per-goal history: total completion count, last completed date, and a mini activity indicator
- [x] **PROG-03**: Micro-milestone celebrations (confetti or animated badge) fire when a goal's cumulative completion count reaches thresholds: 5, 10, 25, 50
- [x] **PROG-04**: A momentum score (completions in last 7 days ÷ 7, clamped 0–1) is computed per goal and shown in GoalDetailView with a color indicator
- [x] **PROG-05**: All progress and momentum computations derive from existing `CompletionEvent` records — no new model required

### UI Additions & Fixes (Phase 15)

- [x] **UIADD-01**: HomeView additions — "Log your workout →" CTA below primaryGoalCard (shown only when not checked in today), Quick Stats row with active goals / total check-ins / badges earned, Stay Close section with About Us / Contact Us / FAQ cards
- [x] **UIADD-02**: Support pages — `AboutUsView` (static scroll with terra gradient header + founder note) and `FAQView` (DisclosureGroup accordion, 8–10 Q&As) in `Views/Support/`
- [x] **UIADD-03**: ChallengeDiscoveryView full redesign — replace flat layout with: VitaminDispenserView (shake gesture → random goal), MoodScannerView (mood pill filter), TrendingChallengesRow (horizontal scroll of featured cards), VitaminShelfGrid (2-column category grid); Build-Your-Own button moved below Mood Scanner; GoalViewModel.addGoal returns `Goal` and navigates to new goal after add
- [x] **UIADD-04**: Community Goals landing page — `CommunityGoalsLandingView.swift` (new): collective progress ring, photo wall with PhotosPicker + CloudKit CKAsset posting, live ticker (seed data), leaderboard (local); `CommunityChallengeCellView` redesigned (100pt height, 56pt icon square, ProgressRingView); `AppRoute.communityGoals(UserChallenge)` case added; card tap routes to landing instead of community feed
- [x] **UIADD-05**: ProfileView fixes — mood picker hidden after today's mood logged (replaced by "Feeling: [mood]" badge with "change" link); camera badge wired to request `AVCaptureDevice` permission then present `PhotosPicker`; on photo selection, compress to max 200KB JPEG and save to `profile.photoData`
- [x] **UIADD-06**: WelcomeScreen auth buttons — `SignInWithAppleButton` (AuthenticationServices, black pill); Google Sign-In stub (white pill + inline Google G SVG, shows "coming soon" alert on tap, zero new dependencies); "I'll set this up later" ghost link moved below auth buttons
- [x] **UIADD-07**: `UserProfile` gains `username: String?` field via SchemaV6 migration (V5→V6, nil default on existing records); displayed as "@username" alongside display name in ProfileView and ProfileEditSheet; editable in ProfileEditSheet with validation (alphanumeric + underscores, lowercase, max 30 chars); no `@Attribute(.unique)` (CloudKit restriction)
...
| ONBOARD-01 | Phase 5 | Complete |
| ONBOARD-02 | Phase 5 | Complete |
| ONBOARD-03 | Phase 5 | Complete |
| ONBOARD-04 | Phase 5 | Complete |
...
| CHAL-13 | Phase 14 | Complete |
| CHAL-14 | Phase 14 | Complete |
| CHAL-15 | Phase 14 | Complete |
| CHAL-16 | Phase 14 | Complete |
| CHAL-17 | Phase 14 | Complete |
| CHAL-18 | Phase 14 | Complete |
| CHAL-19 | Phase 14 | Complete |
| CHAL-20 | Phase 14 | Complete |
| CHAL-21 | Phase 14 | Complete |
| CHAL-22 | Phase 14 | Complete |
| CHAL-23 | Phase 14 | Complete |
| CHAL-24 | Phase 14 | Complete |
| CHAL-25 | Phase 14 | Complete |
| GRAT-01 | Phase 11 | Complete |
| GRAT-02 | Phase 11 | Complete |
| GRAT-03 | Phase 11 | Complete |
| GRAT-04 | Phase 11 | Complete |
| GRAT-05 | Phase 11 | Complete |
| GRAT-06 | Phase 11 | Complete |
| PROG-01 | Phase 12 | Complete |
| PROG-02 | Phase 12 | Complete |
| PROG-03 | Phase 12 | Complete |
| PROG-04 | Phase 12 | Complete |
| PROG-05 | Phase 12 | Complete |

| UIADD-01 | Phase 15 | Complete |
| UIADD-02 | Phase 15 | Complete |
| UIADD-03 | Phase 15 | Complete |
| UIADD-04 | Phase 15 | Complete |
| UIADD-05 | Phase 15 | Complete |
| UIADD-06 | Phase 15 | Complete |
| UIADD-07 | Phase 15 | Complete |
...
*Last updated: 2026-05-15 — all checkboxes and traceability synced to reflect 15 completed phases*
