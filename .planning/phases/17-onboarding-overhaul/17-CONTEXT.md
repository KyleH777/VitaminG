...
- **D-12 (REVISED 2026-05-17):** Report/Block delivered on `PublicProfileView` (the sheet shown when viewing another user's profile) as the primary Phase 17 target — this is where App Store Guideline 1.2 compliance actually applies. `ProfileView` (self-view) is NOT modified. Phase 22 carries the pattern forward for the full public profile redesign. Original D-12 said "ProfileView only" but that was incorrect — ProfileView is the self-view and Report/Block on a self-view is non-functional. User confirmed PublicProfileView is the correct Phase 17 scope.
