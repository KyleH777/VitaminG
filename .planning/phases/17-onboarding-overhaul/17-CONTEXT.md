...
- **D-06:** New onboarding flow (in order):
  ...
  GoalCreationWizardView onboarding step: planner decides whether to keep or remove (Phase 18 redesigns goal creation wizard).
...
### T&C Acknowledgment
- **D-08:** T&C PDF (`Vitamin_G_Terms_and_Conditions.pdf`) is bundled in the app bundle. The T&C step shows a brief description and a "Read Terms" button that opens the PDF via QuickLook in a sheet modal. User dismisses the sheet to return to the step.
...
- **D-14:** Report action: opens a pre-filled email via `MFMailComposeViewController` (or `mailto:` URL fallback) with subject `Report User: [username]` and body pre-populated with context (reporter's Apple user ID, reported user's username, timestamp).
...
### Claude's Discretion
- Whether `GoalCreationWizardView(isOnboarding: true)` remains as the final onboarding step or is removed — given Phase 18 redesigns goal creation, planner decides.
...
- StepBarView `total:` count update to match the new step count.
- Whether to use `MFMailComposeViewController` or a `mailto:` URL for the Report action (device may not have Mail configured).
...
- `VitaminG/VitaminG/VitaminG/Views/Onboarding/OnboardingView.swift` — add new cases to `OnboardingStep` enum (`.termsAndConditions`, `.username`, `.profilePicture`, `.cameraPermission`); remove `.motivationCategories` case; update `navigationDestination` wiring; reorder steps
- `VitaminG/VitaminG/VitaminG/Views/Onboarding/OnboardingViewModel.swift` — extend to manage T&C acknowledgment state, username validation async task, photo data
...
- `VitaminG/.../Views/Onboarding/UsernameScreen.swift` — new username claim step with debounced CKQuery
...
- `StepBarView`: Reusable step progress indicator — update `total:` count when new steps are added.
...
- `OnboardingStep: Hashable` enum + `NavigationStack(path:)` — the step router; add 4 new cases, remove 1.
...
- CloudKit async query pattern for username check: `CKDatabase.perform(CKQuery(...))` or `CKDatabase.records(matching:)` — researcher to confirm recommended API for iOS 17+.
- `MFMailComposeViewController` availability check (`MFMailComposeViewController.canSendMail()`) before presenting; `mailto:` URL fallback if not available.
...
- UsernameScreen: Inline ✓/✗ feedback beneath the text field (500ms debounce); use a subdued color (VGTheme.sage for available, VGTheme.terra for taken); Continue button uses `.disabled()` binding.
...
- GoalCreationWizardView onboarding integration — Phase 18 redesigns the full goal creation wizard; planner decides whether to keep the `isOnboarding: true` variant in the Phase 17 flow.
